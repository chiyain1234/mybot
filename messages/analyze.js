...
const handleAnalyze = async (message) => {
    if (message.author.bot || !message.content.startsWith(".analyze")) return false;
    ...
    if (args[1]) {
        ...
        if (mentionMatch) {
            ...
            await message.reply("正しいユーザーIDまたはメンションを指定してください。例: `.analyze 123456789012345678` または `.analyze @ユーザー`");
            return true;
        }
    } else {
    ...
    try {
        ...
        const prompt = `以下は、あるDiscordユーザーの発言内容の抜粋です。突然の話題の転換やコマンド使用については議論しないでください．\n` +
            `このユーザーがどういった性格・思考傾向・関心を持っているのか、日本語で簡潔に分析してください。\n` +
            `ネガティブな断定は避け、客観的かつ丁寧な文章にしてください。\n\n` +
            `【発言ログ】\n${combinedText}`;
        ...
        const chatCompletion = await groq.chat.completions.create({
            ...
            messages: [
                {
                    role: "system",
                    content: "あなたはユーザーの発言から性格や傾向を分析する日本語アシスタントです。冷静かつ客観的に表現してください。",
                },
                {
                    role: "user",
                    content: prompt,
